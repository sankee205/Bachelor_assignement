...
class _UpdatePopularListsState extends State<UpdatePopularLists> {
  ...
  _fromMapToBoardList() {
    ...
    if (_popularCases != null && _allCases != null) {
      for (int i = 0; i < _popularCases.length; i++) {
        var caseObject = _popularCases[i];
  ...
  Future<bool> _createNewPopularCaseList(List<String> newList) async {
    ...
    var result = await _db.updateFolder('PopularCases', listToFirebase);
    if (result != null) {
      return true;
    } else {
      return false;
